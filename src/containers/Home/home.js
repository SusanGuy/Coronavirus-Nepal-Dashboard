...
import { handleMerge, calculateAdditional, caclulateTimeSeries, getTotalDiff } from '../../utils';
...
const Home = () => {
    ...
    useEffect(() => {
        ...
        const getStates = async () => {
            try {
                const [ { data: totalData }, { data }, { data: ownData } ] = await Promise.all([
                    axios.get('/'),
                    axios.get('/covid'),
                    axios.get('/chart')
                ]);
    ...
    if (fetched) {
        ...
        districtCases = sahiCases.map(({ date, provinceId, provincename, ...rest }) => {
            return {
                ...rest
            };
        });
        ...
        Object.keys(heroCases).forEach((province) => {
            daiCases.push({
                ...
                name: totalCases.find(({ provinceId }) => province.toString() === provinceId.toString()).provinceName,
        ...
        daiCases.forEach(({ id, name, districts }) => {
            provinceCases.push({
                ...
                additionalRecovery: calculateAdditional(districts, 'additionalRecovery'),
        ...
        facts = {
            ...
            recovered: districtCases.reduce((init, current) => init + current.recovered, 0),
            ...
            newTotal: districtCases.reduce((init, current) => init + current.additionalTotal, 0),
            newRecovered: districtCases.reduce((init, current) => init + current.additionalRecovery, 0),
            newDeath: districtCases.reduce((init, current) => init + current.additionalDeaths, 0),
            newActive: districtCases.reduce((init, current) => init + current.additionalActive, 0)
        };
                ...
                {!fetched ? (
                    ...
                    <Fragment>
                        ...
                        <div className='home-right'>
                            <MapViewer prov={provinceCases} dist={districtCases} {...facts} date={date} />

                            <Dome ownData={totalData} groupedTimeline={groupedTimeline} />
                        </div>
                    </Fragment>
                )}
            </div>
